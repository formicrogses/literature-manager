...
class LiteratureManager {
    ...
    setupEventListeners() {
        ...
        // GitHub configuration events
        document.getElementById('configGitHub').addEventListener('click', () => {
            this.showGitHubConfig();
        });

        document.getElementById('closeGitHubConfig').addEventListener('click', () => {
        ...
        document.getElementById('testGitHubConfig').addEventListener('click', () => {
            this.testGitHubConnection();
        });

        document.getElementById('cancelGitHubConfig').addEventListener('click', () => {
            this.hideGitHubConfig();
        });
    ...
    async processFileItem(fileItem, selectedCategory) {
        try {
            ...
            // More realistic progress updates with status text
            ...
            }, 600); // Slower updates for better UX
            ...
            const paperData = await Promise.race([parsePromise, timeoutPromise]);
    ...
    async compressPDF(file) {
        try {
            // Method 1: Use PDF.js to re-render PDF with lower quality
            if (typeof pdfjsLib !== 'undefined') {
                return await this.compressPDFWithRenderer(file);
            } else {
                // Method 2: Simple compression by reducing base64 quality (fallback)
                return await this.compressPDFSimple(file);
            }
        } catch (error) {
    ...
    async compressPDFWithRenderer(file) {
        const arrayBuffer = await file.arrayBuffer();
        const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise;
        
        // Create a new PDF by re-rendering pages with lower quality
        const compressedPages = [];
        const scale = 0.8; // Reduce scale to compress
        const quality = 0.6; // JPEG quality for images
        ...
        for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
            ...
            compressedPages.push(imageData);
        }
        
        // Since we can't create a real PDF from images easily in browser,
        // we'll use a different approach: reduce the base64 size
        const originalBase64 = await this.convertFileToBase64(file);
        const compressedBase64 = await this.compressBase64(originalBase64, 0.7);
        
        // Create a new File object with compressed data
        const compressedData = this.base64ToBlob(compressedBase64);
    ...
    async compressPDFSimple(file) {
        // Simple compression by reducing quality of base64 representation
        const base64 = await this.convertFileToBase64(file);
        const compressedBase64 = await this.compressBase64(base64, 0.6);
    ...
    async compressBase64(base64String, quality = 0.7) {
        // Extract the actual base64 data (remove data:application/pdf;base64, prefix)
        const base64Data = base64String.split(',')[1];
        const binaryString = atob(base64Data);
        
        // Simple compression: take every nth byte based on quality
        const compressionRatio = Math.max(0.3, quality);
        const targetLength = Math.floor(binaryString.length * compressionRatio);
        const step = binaryString.length / targetLength;
        
        let compressedBinary = '';
        for (let i = 0; i < targetLength; i++) {
            const index = Math.floor(i * step);
            compressedBinary += binaryString[index];
        }
    ...
    showGitHubConfig() {
        ...
        if (window.GITHUB_CONFIG.token !== 'YOUR_GITHUB_TOKEN') {
            document.getElementById('githubToken').value = window.GITHUB_CONFIG.token;
        }
    ...
    updateSyncBanner(message, type = 'info') {
        ...
        if (type === 'loading') {
            ...
            textElement.innerHTML = `⚠️ ${message}`;
        } else if (type === 'error') {
            textElement.innerHTML = `❌ ${message}`;
        } else {
            textElement.innerHTML = `💡 ${message}`;
        ...
        if (type === 'success') {
            ...
            banner.style.background = 'linear-gradient(135deg, #dc3545 0%, #e83e8c 100%)';
